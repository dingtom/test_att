...
class SAAConvLSTMCell(nn.Module):
    ...
    # Arguments:
        ...
        padding: padding
    '''
    def __init__(self, input_channels, hidden_channels, kernel_size, num_past_frames, 
    dk, dv, Nh, width, height, 
    attention_input_mode='representation', positional_encoding=True, forget_bias=1.0):
        ...
        self.W_hi = nn.Conv2d(self.hidden_channels, self.hidden_channels, self.kernel_size, stride=1, self.padding, bias=True)

        self.W_xf = AAConv2d(self.input_channels, self.hidden_channels, self.kernel_size, dk, dv, Nh)
        self.W_hf = nn.Conv2d(self.hidden_channels, self.hidden_channels, self.kernel_size, stride=1, self.padding, bias=True)
        
        self.W_xc = AAConv2d(self.input_channels, self.hidden_channels, self.kernel_size, dk, dv, Nh)
        self.W_hc = nn.Conv2d(self.hidden_channels, self.hidden_channels, self.kernel_size, stride=1, self.padding, bias=True)
        
        self.W_xo = AAConv2d(self.input_channels, self.hidden_channels, self.kernel_size, dk, dv, Nh)
        self.W_ho = nn.Conv2d(self.hidden_channels, self.hidden_channels, self.kernel_size, stride=1, self.padding, bias=True)
